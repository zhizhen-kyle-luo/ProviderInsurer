...
sources: WV BMS ASP (drugs), WV BMS CLFS (labs), CMS PFS benchmarks (procedures), CAQH 2023 (admin).
...
DRUG_CODES: Dict[str, CodeInfo] = {
    ...
    "Q5103": CodeInfo(612.15, "Injection, infliximab biosimilar variant, 10 mg", ["infliximab"]),
...
PROCEDURE_CODES: Dict[str, CodeInfo] = {
    "96413": CodeInfo(130.00, "Chemotherapy administration, IV infusion, up to 1 hour", ["infusion", "chemo", "iv"]),
    "96415": CodeInfo(40.00, "Chemotherapy administration, IV infusion, each additional hour", ["infusion", "chemo", "iv"]),
    "96365": CodeInfo(70.00, "IV infusion, therapeutic/diagnostic, up to 1 hour", ["infusion", "iv"]),
    "96366": CodeInfo(25.00, "IV infusion, therapeutic/diagnostic, each additional hour", ["infusion", "iv"]),
    "74183": CodeInfo(350.00, "MRI abdomen without contrast, then with contrast", ["mri", "abdomen"]),
    "72197": CodeInfo(350.00, "MRI pelvis without contrast, then with contrast", ["mri", "pelvis"]),
}

ADMIN_COST_PROVIDER_PER_TURN: float = 5.79
ADMIN_COST_INSURER_PER_TURN: float = 0.05
